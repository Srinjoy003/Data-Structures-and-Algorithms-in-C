...
void insert(int arr[][3],int cost[][100],int n,int element1,int element2,int* size){ 
    arr[n][0] = element1;
    arr[n][1] = element2;
...
void delete(int arr[][3],int cost[][100],int n,int* size){
...
int MST(int cost[][100],int v){
    ...
    for(int i = 0;i < v; i++){
        if(cost[curr][i] != I && cost[curr][i] != 0  && visited[i] != 1){
            insert(Q,cost,size,i,curr,&size); 
        }
    }
    ...
    for(int i = 0; i < v-1;i++){;
        ...
        printf("%d -----%d----- %d \n",tree[i][0],cost[tree[i][0]][tree[i][1]],tree[i][1]);
    }
